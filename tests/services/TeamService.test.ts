...
describe('TeamService', () => {
  ...
  it('updateTeam should modify an existing team', () => {
    ...
    const team = TeamService.createTeam(initialName);
  ...
  it('createTeam should fail if name is empty', () => {
    expect(() => TeamService.createTeam('')).toThrow('Team name is required.');
    expect(() => TeamService.createTeam('   ')).toThrow('Team name is required.');
  });
  ...
  it('createTeam should fail if name is not unique', () => {
    ...
    expect(() => TeamService.createTeam(name)).toThrow(`Team name "${name}" is already taken.`);
  });

  it('updateTeam should fail if name is empty', () => {
    const team = TeamService.createTeam('Valid Name');
    expect(() => TeamService.updateTeam(team.id, '')).toThrow('Team name is required.');
  });
  ...
  it('updateTeam should fail if name is taken by another team', () => {
    ...
    expect(() => TeamService.updateTeam(team2.id, 'Team 1')).toThrow(
  ...
  it('deleteTeam should remove the team from the database', () => {
    const team = TeamService.createTeam('To Be Deleted');
    const id = team.id;
  ...
  it('createTeam should generate a team with Start and End nodes and a path', () => {
    const team = TeamService.createTeam('Service Test Team');
  ...
  it('updateTeam should reject teams without a Start node', () => {
    const team = TeamService.createTeam('Missing Start');
    const agents = team.agents.filter((a) => a.persona_name !== 'Start');
    expect(() => TeamService.updateTeam(team.id, team.name, agents, team.connections)).toThrow(
      'Team must include exactly one Start node.'
    );
  });

  it('updateTeam should reject teams without an End node', () => {
    const team = TeamService.createTeam('Missing End');
    const agents = team.agents.filter((a) => a.persona_name !== 'End');
    expect(() => TeamService.updateTeam(team.id, team.name, agents, team.connections)).toThrow(
      'Team must include exactly one End node.'
    );
  });
  ...
  it('getNextAgents should return the next ready agent in a simple path', () => {
    const team = TeamService.createTeam('Next Agents Simple');
    const start = team.agents.find((a) => a.persona_name === 'Start')!;
    const end = team.agents.find((a) => a.persona_name === 'End')!;
    ...
    TeamService.updateTeam(team.id, team.name, agents, connections);
  ...
  it('getNextAgents should return multiple ready agents when branching', () => {
    const team = TeamService.createTeam('Next Agents Branch');
    const start = team.agents.find((a) => a.persona_name === 'Start')!;
    const end = team.agents.find((a) => a.persona_name === 'End')!;
