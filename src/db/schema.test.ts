...
describe("Database Schema", () => {
  ...
  it("should create a team correctly", () => {
    ...
    testDb.prepare("INSERT INTO teams (id, name, objective) VALUES (?, ?, ?)").run(
      id,
      "Test Team",
      "Test Objective"
    );

    const team = testDb.prepare("SELECT * FROM teams WHERE id = ?").get(id) as any;
  ...
  it("should cascade delete agents when team is deleted", () => {
    ...
    testDb.prepare("INSERT INTO agents (id, team_id, role) VALUES (?, ?, ?)").run(
      agentId,
      teamId,
      "Tester"
    );
